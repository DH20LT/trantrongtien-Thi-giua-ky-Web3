...
def new_task_by_project(projectId):
    ...
    if is_logged_in():
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            kiem_tra_neu_tat_ca_nhiemvu_trong_project_hoan_thanh(projectId)
        ...
        if _task_id:
            if form.submitUpdate.data:
                ...
                if _status.desc == 'Đang thực hiện':
                    cap_nhat_trang_thai_project(projectId, _status_id)
                ...
                kiem_tra_neu_tat_ca_nhiemvu_trong_project_hoan_thanh(projectId)
                return render_template('new-task.html', form=form, user=user, task=task, is_logged_in=is_logged_in())
        elif form.validate_on_submit():
            print('Form validated')
...
def cap_nhat_trang_thai_project(project_id, status_id):
    project = db.session.query(models.Project).filter_by(project_id=project_id).first()
    project.status_id = status_id
    db.session.commit()


def kiem_tra_neu_tat_ca_nhiemvu_trong_project_hoan_thanh(project_id):
    ...
    for task in tasks:
        if task.status.desc == 'Hoàn thành':
            count += 1
    if count == len(tasks):
        cap_nhat_trang_thai_project(project_id, 4)
    else:
        cap_nhat_trang_thai_project(project_id, 2)
...
def project_detail(projectId):
    ...
    return render_template('project_detail.html', user=user, project=project, is_logged_in=is_logged_in())
